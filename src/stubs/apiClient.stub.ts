...
export async function request(path: string, options?: { method?: string; body?: any }) {
  // Usuarios demo
  const demoUsers: Record<string, { username: string; role: UserRole; areas?: number[]; equipos?: number[] }> = {
  ...
  if (path === '/auth/refresh' && options?.method === 'POST') {
    const body = typeof options.body === 'string' ? JSON.parse(options.body) : options.body || {};
    // Simula refresco: devuelve nuevo token y refresh_token, y actualiza expiresAt
    const expiresIn = 3600;
    const expiresAt = Date.now() + expiresIn * 1000;
    // El usuario puede venir en el body, si no, usa superadmin
    const user = body.user || demoUsers['superadmin'];
    return {
      token: 'demo-token',
      refresh_token: 'demo-refresh-token',
      expiresAt,
